...
Nothing is timed against the page's own clock, and that is the point of holding a connection rather than counting ticks. A browser slows the timers of a window that is not in front of you, to about one a minute once it has been out of sight for five minutes, and stops them altogether while the machine is asleep. A reading that ended when those ticks dried up would die behind its own window while it was merely minimised, and what you would find on coming back is a page that looks perfectly well and does nothing at all: no redraw when the file changes, no `Edit`, and nothing behind a reload. A connection sleeps as happily as the machine it is on, so a reading survives a window left in the background for an hour and a laptop shut for a night. A reading no page has ever held is left alone as well, so one opened where no browser could draw it goes on serving and stays reachable by hand.
