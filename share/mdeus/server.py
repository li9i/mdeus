...
def load_state():
    ...
    try:
        ...
        if stored['theme'] in THEMES:
            return {'contents': bool(stored['contents']), 'theme': stored['theme']}
    except (KeyError, OSError, TypeError, ValueError):
        pass
    return {'contents': False, 'theme': 'serif'}
...
def page_html(name, theme, vim):
    ...
    sync = (
        '\n    <link rel="stylesheet" href="/assets/cvim.css" />'
        '\n    <script src="/assets/cvim.js" defer></script>'
        if vim
        else ''
    )
